							...
							<!-- Section -->
								<section>
									...
									<div style="position:relative;" >
										<form action="{{ url('app_report') }}" method="get" style="color:black;">
                                                        @csrf
									...
									@if(count($appoint) > 0)
									{{$appoint->appends(['ddate' => request('ddate'), 'period' => request('period')])->links()}}
									<div id="print3">
										<h1 class="title-style">Appointments</h1><br>
										...
										@foreach($appoint as $appoints)	
											<tr>
												<td>{{$appoints->name}}</td>
												<td>{{$appoints->patient_type}}</td>
												<td>{{ date ('F d, Y',strtotime($appoints->date)) }} <br>
													{{$appoints->slot}}</td>
												<td>{{$appoints->doctor}}</td>
												<td>{{$appoints->message}}</td>
												<td style="color: black;text-align:center">
													@if($appoints->status === 'Denied')
													<p style="background-color:red">{{$appoints->status}}</p>
													@elseif($appoints->status ==='Approved')
													<p style="background-color:green;">{{$appoints->status}}</p>
													@else {{$appoints->status}}
													@endif
												  </td>
												<td>{{ date('F d,Y h:ia', strtotime($appoints->created_at)) }}</td>
											</tr>
										@endforeach	
										</table>
									</div>
									@else
									<div style="display: none;" id="print3"></div>
									@endif

									<div id="report-footer" style="display: none;justify-content:space-between;">
										<h5>Prepared By: <br> <br><br> Hydie Ver Casihan <br>Secretary</h5>
										<h5>Noted By: <br> <br><br> Francess Carmel Birung, MD <br>Ob-Gyne Doctor <br><span>Lic. No. 127449</span></h5>
									</div>
		...
		<!-- Scripts -->
			...
			<script>
				...
				function printNow() {
				  ...
				  var data3 = document.getElementById('print3').innerHTML;
				  ...
				  document.getElementById('body').innerHTML = '<div class="header">' + header  + '</div>'+ data3 +'<div class="footer">'+footer+ '</div>';
				  
				  // Set the isPrinting flag to true
				  isPrinting = true;
				  
				  window.print(header +data3 + footer);
				}
