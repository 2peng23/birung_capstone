							...
							<!-- Section -->
								<section>
									...
									<div style="position:relative;" >
										<form action="{{ url('ob_report') }}" method="get" style="color:black;">
                                                        @csrf
									...
									@if(count($ob) > 0)
									{{$ob->appends(['ddate' => request('ddate'), 'period' => request('period')])->links()}}
									<div id="print2">
										<h1 class="title-style">Ob-Gyne Patients</h1><br>
										...
										@foreach($ob as $obs)	
											<tr>
												...
												<td>{{$obs->address2}}</td>
												<td>{{$obs->diag2}}</td>
												<td>{{ date('F d,Y h:ia', strtotime($obs->updated_at)) }}</td>
											</tr>
										@endforeach	
										</table>
									</div>
									@else
									<div style="display: none;" id="print2"></div>
									@endif
									...
									<div id="report-footer" style="display: none;justify-content:space-between;">
										...
										<h5>Noted By: <br> <br><br> Francess Carmel Birung, MD <br>Ob-Gyn <br> <br><span>Lic. No. 127449</span></h5>
									</div>
		...
		<!-- Scripts -->
			...
			<script>
				...
				function printNow() {
				  ...
				  var data2 = document.getElementById('print2').innerHTML;
				  ...
				  document.getElementById('body').innerHTML = '<div class="header">' + header  + '</div>' +  data2  +'<div class="footer">'+footer+ '</div>';
				  
				  // Set the isPrinting flag to true
				  isPrinting = true;
				  
				  window.print(header + data2 + footer);
				}
