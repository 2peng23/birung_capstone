									...
									<div class="features">
										<div style="align:center;" >
                                            ...
                                            <div style=" width:98%; margin:10px auto;display:flex;justify-content:space-between;">
                                              <div>
                                                <form action="{{url('search-ob-record')}}" method="post" style="color: black;  margin-top:10px;">
                                                  {{csrf_field()}}
                                                  <input type="search" name="nname" autocomplete="off" style="border-color:black;width:200px;" placeholder="Search Patient">
                                                  <button  type="submit" style="color:black;height:43px;width:200px;margin-top:10px" hidden>Search Patient</button>
                                                </form>
                                                <form method="GET" action="{{ url('/obpatient-list') }}">
                                                  <input type="number" name="pagesize" id="" required placeholder="Pagination" style="width:200px;border-radius:5px">
                                                  <input type="submit" value="Submit" hidden>
                                              </form>
                                              </div>
                                
                                              <div style="text-align: center;"> 
                                                  <a class="btn btn-success" href="{{url('add-ob-patient')}}" style="border:none;width:180px">Add New Patient  +</a>  <br>
                                                  <a class="btn btn-primary" href="{{url('archive-obpatient')}}" style="border:none;margin-top:50px;width:180px;">Inactive Patient</a>
                                              </div>
                                          ...
                                          <div id="printable">
                                            <table class="table-style" style="border:solid black 1px; margin-left: 10px; margin-right: 10px;">
                                                ...
                                                $sortedData = $data2->sortByDesc('updated_at');
                                                @endphp
                                            
                                            @foreach($sortedData as $patientdata2)
                                                <tr style="color: black; border:1px solid black;">
                                                    <td>{{$patientdata2 ->name2}}</td>
                                                    <td>{{$patientdata2 ->email2}}</td>
                                                    <td>{{$patientdata2 ->phone2}}</td>
                                                    <td>{{$patientdata2 ->age2}}</td>
                                                    <td>{{$patientdata2 ->sex2}}</td>
                                                    <td>{{$patientdata2 ->address2}}</td>
                                                    @php date_default_timezone_set('Asia/Manila'); @endphp
                                                    <td>{{ date('M j, Y ', strtotime($patientdata2->bday2)) }}</td>
                                                    
                                                    @php date_default_timezone_set('Asia/Manila'); @endphp
                                                    <td>{{ date('M j, Y h:i A', strtotime($patientdata2->updated_at)) }}</td>
                                                    <td style="text-align:center;" class="no-print">
                                                      <a href="{{url('viewobpatient', $patientdata2->id)}}" class="btn btn-success" style="width:80px">View</a> 
                                                      <a href="{{url('editobpatient', $patientdata2->id)}}" class="btn btn-primary" style="width:80px">Update</a>
                                                    </td>
                                                </tr>
                                            @endforeach
                                            </table>
